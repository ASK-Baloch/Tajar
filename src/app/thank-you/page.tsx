        ...
        <div className="mx-auto max-w-2xl px-4 py-16 sm:px-6 sm:py-24 lg:grid lg:max-w-7xl lg:grid-cols-2 lg:gap-x-8 lg:px-8 lg:py-32 xl:gap-x-24">
          <div className="lg:col-start-2 ">
            ...
            {order._isPaid ? <p className="mt-2 text-base text-muted-foreground">
              ...
              your order has been placed successfully and is on its way.We&apos;ll email  the details and receipt to {" "} {typeof order.user !== "string" ? (<span className="font-medium text-gray-900">{order.user.email}</span>) : null }.
            </p> : <p className="mt-2 text-base text-muted-foreground">
              we appreciate your order.We&apos;re currently Processing it. So hang tight and  We&apos;ll email you for confirmation when your order is Placed</p>}
                  ...
                  {(order.products as Product[]).map((product) => {
                    ...
                    return <li key={product.id} className="flex space-x-6 py-6">
                      <div className="relative h-24 w-24 ">
                        {typeof image !== 'string' && image.url ?  (<Image src={image.url} fill alt={`${product.name} Image `} className="flex-none rounded-md bg-gray-100 object-cover object-center " />) : null}
                      </div>
